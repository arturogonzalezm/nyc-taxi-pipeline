-- Sample Analytics Queries for NYC Taxi Data Warehouse (BigQuery)
...
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips`
...
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips` f
JOIN `PROJECT_ID.nyc_taxi_warehouse.dim_datetime` d
...
SELECT
    IF(d.is_weekend, 'Weekend', 'Weekday') as day_type,
    COUNT(*) as trip_count,
    ROUND(AVG(f.total_amount), 2) as avg_fare,
...
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips` f
JOIN `PROJECT_ID.nyc_taxi_warehouse.dim_payment` p
...
SELECT
    ...
    COUNT(*) as trip_count
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips` f
JOIN `PROJECT_ID.nyc_taxi_warehouse.dim_location` l
...
-- Query 9: Peak hours analysis with partitioning
SELECT
    d.hour,
    d.is_weekend,
    COUNT(*) as trip_count,
...
JOIN `PROJECT_ID.nyc_taxi_warehouse.dim_datetime` d
    ON f.datetime_key = d.datetime_key
WHERE f.data_source_month = '2023-01'  -- Partition pruning
...
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips`
UNION ALL
SELECT 'Unique datetime keys', COUNT(DISTINCT datetime_key)
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips`
UNION ALL
SELECT 'Unique pickup locations', COUNT(DISTINCT pickup_location_key)
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips`
UNION ALL
SELECT 'Unique dropoff locations', COUNT(DISTINCT dropoff_location_key)
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips`
UNION ALL
SELECT 'Avg trip duration (minutes)', ROUND(AVG(trip_duration_minutes), 2)
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips`
UNION ALL
SELECT 'Avg fare amount', ROUND(AVG(total_amount), 2)
FROM `PROJECT_ID.nyc_taxi_warehouse.fact_trips`;
